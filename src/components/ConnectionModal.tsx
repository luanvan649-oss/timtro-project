...
function ConnectionModal({ isOpen, onClose, post, targetUser, currentUser }: ConnectionModalProps) { // Add currentUser prop
  const { socket } = useSocket() as any; // Use the socket hook
  ...
  useEffect(() => {
    if (socket) {
      socket.on('connectionRequestSent', (data: any) => {
      ...
      socket.on('connectionRequestFailed', (errorMsg: any) => {
